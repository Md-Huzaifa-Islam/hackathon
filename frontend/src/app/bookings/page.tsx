...
import { bookings as bookingFixtures } from "@/data/bookings";
...
export default function BookingsPage() {
  ...
  const movieMap = useMemo(() => new Map((data ?? []).map((movie) => [movie.id, movie])), [data]);

  const filteredBookings = useMemo(() => {
    const today = new Date();
    return bookingFixtures.filter((booking) => {
      const status = booking.status.toUpperCase();
      if (activeTab === "Cancelled") return status === "FAILED" || status === "EXPIRED" || status === "REFUNDED";
